...
func (s *MessageService) Send(actor *models.User, roomID int64, text string, fileID *int64, opts *SendOptions) (*models.MessageView, error) {
	if len(text) > MaxMessageChars {
		return nil, fmt.Errorf("message too long (max %d characters)", MaxMessageChars)
	}
...
func (s *MessageService) CreatePoll(actor *models.User, roomID int64, question string, options []string) (*models.MessageView, *models.Poll, error) {
	...
	if len(question) > 500 {
		return nil, nil, errors.New("سوال نظرسنجی حداکثر ۵۰۰ کاراکتر")
	}
	...
	for i, opt := range options {
		if len(opt) > 200 {
			return nil, nil, errors.New(fmt.Sprintf("گزینه %d بیش از ۲۰۰ کاراکتر است", i+1))
		}
	}
...
// SearchMessagesPaged searches with limit + offset for pagination
func (s *MessageService) SearchMessagesPaged(actor *models.User, query string, limit, offset int) ([]models.MessageView, error) {
	if query == "" {
		return []models.MessageView{}, nil
	}
	all, err := s.SearchMessages(actor, query, 1000) // get all matches
	if err != nil {
		return nil, err
	}
	if offset >= len(all) {
